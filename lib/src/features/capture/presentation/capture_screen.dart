...
class _CaptureScreenState extends ConsumerState<CaptureScreen> {
  ...
  void _startAutoCapture() {
    ...
    _autoCaptureTimer = Timer.periodic(const Duration(milliseconds: 1000), (timer) {
      if (_stability > 0.7 && _photoCount < _targetPhotos) {
        _takePhoto();
      }
    });
  }
  ...
  Future<void> _finalizeCapture() async {
    ...
    try {
      ...
      final projectId = await repository.createProject(
        ...
        description: 'Captured via mobile app',
        ownerPhone: 'user_phone_here', // In a real app, get from auth state
        fcmToken: _fcmToken,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Upload complete! You will be notified when the 3D model is ready.')),
        );
        context.go('/home');
      }
    } catch (e) {
